...
describe('mergeLanguages', () => {
  it('language numbers should match up', () => {
    const result = mergeLanguages(repository, languages);
    expect(Object.keys(result.languages).length).toBe(Object.keys(languages).length);
    expect(Object.keys(result.languages).length).toBe(Object.keys(repository.languages.nodes).length);
  });
  it('sizes should match up', () => {
    const result = mergeLanguages(repository, languages);
    expect(result.languages.JavaScript.size).toBe(1000);
  });
  it('should have color', () => {
    const result = mergeLanguages(repository, languages);
    expect(result.languages.JavaScript.color).toBe('#f1e05a');
  });
});